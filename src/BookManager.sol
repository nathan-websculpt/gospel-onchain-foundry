...
contract BookManager is Ownable {
    ...
    function _storeVerse(
        ...
        emit Verse(msg.sender, _bookId, numberOfVerses, _verseNumber, _chapterNumber, _verseContent);
    }

    // verse-skip prevention
    // to prevent skipping verses
    // prevents the situation of storing 1:1 and then storing 1:3
    function preventSkippingVerse(uint256 _verseNumber, uint256 _chapterNumber) private view returns (bool) {
        bool canContinue = true;
